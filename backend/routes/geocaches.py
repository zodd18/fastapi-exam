...
async def read_geocaches(hint: Optional[str] = None, email: Optional[str] = None):
    ...
    if hint:
        geocaches = geocache_service.get_geocaches_by_hint(hint)
    elif email:
        geocaches = geocache_service.get_logbooks_by_email(email)
    else:    
        geocaches = geocache_service.get_all_geocaches()
...
async def read_geocache(geocache_id):
    geocache = geocache_service.get_geocache(geocache_id)
...
async def get_logbooks_by_geocache(geocache_id):
    logbooks = logbook_service.get_logbooks_by_geocache(geocache_id)
...
async def get_geocaches_not_found():
    geocaches = geocache_service.get_not_found_geocaches()
